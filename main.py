import redis
   ...
   def save_trades():
       try:
           ...
           logger.error(f"Error saving trades to Redis: {e}")
   ...
   def load_trades():
       ...
       try:
           ...
           if data:
               ...
               logger.info(f"Loaded {len(open_trades)} trades from Redis")
           else:
               open_trades = {}
           ...
           logger.error(f"Error loading trades from Redis: {e}")
           open_trades = {}
   ...
   def save_closed_trades(closed_trade):
       try:
           ...
           logger.error(f"Error saving closed trades to Redis: {e}")
       ...
       try:
           ...
           if data:
               ...
               if len(trades) != len(unique_trades):
                   logger.info(f"Removed {len(trades) - len(unique_trades)} duplicate closed trades from Redis")
           ...
           logger.error(f"Error loading closed trades from Redis: {e}")
           return []
   ...
   def edit_telegram_message(message_id, new_text):
       ...
       try:
           ...
           if response.get('ok'):
               ...
               logger.error(f"Telegram edit error: {response.get('description')}")
       except Exception as e:
       ...
       try:
           ...
           with open(filename, 'rb') as f:
               ...
               if response.get('ok'):
                   ...
                   logger.error(f"Telegram send CSV error: {response.get('description')}")
   ...
   def list_files():
       try:
           ...
           logger.info(f"Listing CSV files: {file_list}")
       ...
       try:
           ...
           if not closed_trades_df.empty:
               ...
               if not new_trades_df.empty:
                   ...
                   logger.info(f"Appended {len(new_trades_df)} new closed trades to {CLOSED_TRADE_CSV} and uploaded to GCS")
   ...
   def process_symbol(symbol, alert_queue):
       try:
           for attempt in range(3):
               ...
               if len(candles) < 30:
                   logger.info(f"{symbol}: Skipped, insufficient candles ({len(candles)})")
                   return
           ...
           if ema21 is None or ema9 is None or rsi is None or adx is None or big_candle_rsi is None:
               logger.info(f"{symbol}: Skipped, indicator calculation failed")
               return
           ...
           if not (rising or falling):
               ...
               if is_bullish(c2) and not rising:
                   ...
                   if c1[5] <= c0[5]:
                       reasons.append("Volume not decreasing")
               if reasons:
                   logger.info(f"{symbol}: No pattern detected. Reasons: {', '.join(reasons)}")
               return
   ...
   def run_bot():
       try:
           ...
           while True:
               ...
               export_to_csv()
               logger.info(f"Number of open trades after scan: {len(open_trades)}")
   ...
   def test_redis():
       try:
           ...
           raise  # Re-raise to crash and log
